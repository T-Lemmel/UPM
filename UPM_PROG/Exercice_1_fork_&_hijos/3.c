...
int main(void) {
    int i;
    int status;
    int pid; /* pid_t es un entero que representa el identificador ID de un proceso */
    ...
    switch (pid = fork()) {
        ...
        default:
            ...
            /* Alternativa: la función wait(int *status) provoca una espera del proceso padre hasta que cualquier proceso hijo finalice. 
               La información del estado del proceso hijo se almacena en status. */
            printf("Padre: mi hijo ya terminó!\n");
            break;
    }
    exit(0);
}
